...
export default function ApplicationForm({
  ...
  const [improvement_areas, setimprovement_areas] = useState("")
  const [strengths_summary, setstrengths_summary] = useState("")
  ...
  useEffect(() => {
    if (parsedResumeData || userProfile ) {
  ...
  const handleFileUpload = async (file, type) => {
    if (type === "resume") {
      ...
      formData.append("file", file); // this key must match FastAPI


      try {
        const response = await axios.post(
          `${process.env.NEXT_PUBLIC_FASTAPI_URI}/score-agent`,
          formData,
          {
            headers: {
        ...
        if (jsonMatch && jsonMatch[1]) {
          ...
          try {
            ...
            settotal_score(jsonData.total_score)
            setstrengths_summary(jsonData.strengths_summary)
            setimprovement_areas(jsonData.improvement_areas)
            setsuggestions(jsonData.suggestions)
            // Now you can use jsonData as a proper JavaScript object
            // Example:
            console.log(jsonData.scores.technical_skills); // 28
            console.log(jsonData.total_score); // 92
          } catch (error) {
      ...
      // form.append("name", formData.personalInfo.firstName || "Manil Modi");
      // form.append("certificate", file);
      // const api = await axios.post(`${process.env.NEXT_PUBLIC_FASTAPI_URI}/verify-certificate`, form);

      // const data = api.data;
      // console.log(data);

      // if (data.platform_verified && data.user_name_verified && data.valid_certificate) {
  ...
  const handleSubmit = async () => {
    ...
    const uploadResume = await axios.post("/api/upload", {
      file: resumefile
    }, {
      headers: {
        "Content-Type": "multipart/form-data"
    ...
    const resumeUrl = uploadResume.data.url;
    ...
    const app = await axios.post("/api/job/apply", applicationData)
    const personalInfo = app.data.application.personalInfo
    const obj = {
      ...personalInfo,
      skills : app.data.application.skills
    }
    await axios.put("/api/profile",obj);
    setLoading(false);
